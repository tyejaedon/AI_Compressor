...
This roadmap targets `src/training/train_autoencoder_image_local.py` (lossless/edge-preserving
trainer), `src/training/train_autoencoder_image_lossy_local.py` (rate-distortion trainer),
`src/reporting/upscale_reconstructed_images.py`, `src/reporting/generate_real_image_comparisons.py`,
`src/training/random_search_hyperparams.py` (image paths), and `src/training/param_overrides.py`.
...
| Upscaling | `src/reporting/upscale_reconstructed_images.py` is **classic BICUBIC only** — no learned super-resolution model. |
...
| Search & docs | `src/training/random_search_hyperparams.py` samples both trainers' hyperparameters; `--params-file` JSON convention is shared and documented in `documentation/MODEL_PARAMS_INPUT.md`. |
...
3. **CI stays lightweight.** No training/data-dependent jobs in CI (see `.github/workflows/ci.yml`); all new functionality must remain smoke-testable via `tests/smoke/*.py` with tiny data/epoch counts, run locally by contributors.
4. **No installable-package refactor.** Scripts were reorganized into `src/{training,pipeline,reporting,audio}/` + `tests/smoke/` folders for discoverability, but they remain plain CLI scripts invoked directly with `python src/.../script.py` — no `pyproject.toml`/`setup.py`, no `pip install -e .`, no package namespace. Do not turn this into an installable package.
5. **Backward-compatible CLI.** New flags must have safe defaults that reproduce current behavior; breaking flag renames require a `!`/`BREAKING CHANGE` commit and a `src/pipeline/run_full_production_pipeline.py` sync (per `AGENT.md`).
...
### Milestone 1 — Real Rate-Distortion Accounting ✅ Implemented
...
### Milestone 2 — Quantization-Aware Training & Latent Quality ✅ Implemented
...
4. **[image/lossy] Extend smoke test to assert quantized latents are read back losslessly at chosen bit-depth** — add assertion to `tests/smoke/smoke_test_image_lossy_local.py`.

### Milestone 3 — Learned Upscaling ✅ Implemented
**Objective:** Add an optional learned super-resolution model to replace/augment the BICUBIC-only path in `src/reporting/upscale_reconstructed_images.py`.
...
**Exit criteria:** A new `--upscaler-mode {bicubic,learned}` flag on `src/reporting/upscale_reconstructed_images.py`; a small dedicated SR trainer or reuse of the existing decoder at inference-time with tiling for larger outputs; comparison panels show both.

Issues:
1. **[image/upscale] Design a minimal learned super-resolution head** — small residual CNN trained on the same degrade/upscale-factor pairs already produced by `src/training/train_autoencoder_image_local.py`'s data pipeline.
...
3. **[image/upscale] Wire `--upscaler-mode learned` into `src/reporting/upscale_reconstructed_images.py`** — falls back to BICUBIC if no learned model path is given (`--upscaler-mode bicubic` stays default).
...
5. **[image/docs] Document the learned upscaling workflow end-to-end** — README section + smoke test (`tests/smoke/smoke_test_upscaler_local.py`).
...
**Objective:** Keep `src/training/random_search_hyperparams.py`, `documentation/*.md`, and evaluation reports consistent with milestones 1–4 as they land.
...
Issues:
1. **[image/search] Extend image + image_lossy search spaces for new flags** — bit-depth, rate-loss-mode, quant-noise-anneal, cache-dir toggles added to `src/training/random_search_hyperparams.py` and `documentation/random_search_profile.json`.
...
3. **[image/reporting] Add a compression-ratio/PSNR frontier plot** — extend `src/reporting/plot_training_metrics_from_report.py` / `src/reporting/generate_master_report.py` to chart rate-distortion tradeoff across trials.
4. **[image/pipeline] Sync `src/pipeline/run_full_production_pipeline.py` with any renamed/added flags** — verify end-to-end pipeline still runs after milestones 1–4 land.
